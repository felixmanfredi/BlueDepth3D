...
class GLViewer:
    ...
    def init(self, _argc, _argv, camera_model, res,width=400,height=200,pos_x=0,pos_y=0): # _params = sl.CameraParameters
        ...
        glutInitWindowPosition(int(pos_x), int(pos_y))

        #glutInitWindowPosition(int(wnd_w*0.05), int(wnd_h*0.05))

        glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA | GLUT_DEPTH | GLUT_BORDERLESS)
        ...
        self.bckgrnd_clr = np.array([10/255., 10/255., 10/255.])
    ...
    def updateData(self, pc, mx, my, mz):
        self.mutex.acquire()
    ...
    def keyPressedCallback(self, key,x,y):
        ...
        if key == 27:  # ESC key
            self.close_func()
        elif key == 49:  # Key "1" for top view
            print("pushed 1.")
            self.setCameraView(1)
        elif key == 50:  # Key "2" for side view
            print("pushed 2.")
            self.setCameraView(2)
        elif key == 51:  # Key "3" for front view
            print("pushed 3.")
            self.setCameraView(3)
        elif key == 52:  # Key "0" for front view
                print("pushed 4.")
                self.setCameraView(4)   
        elif key == 87 or key == 119:  # Key "w" for front view
            print("pushed w (top).")
        elif key == 65 or key == 97:  # Key "a" for front view
            print("pushed a (left).")
        elif key == 83 or key == 115:  # Key "s" for front view
            print("pushed s (down).")
        elif key == 68 or key == 100:  # Key "d" for front view
            print("pushed d (right).")
        

    def setCameraView(self, view):
        
        if view == 1:  # Front view Z
            print("Setting front view")
            direction = sl.Translation()
            direction.init_vector(0, 0, -1)  
            vertical = sl.Translation()
            vertical.init_vector(0, 0, 1)  
            self.camera.setDirection(direction, vertical)
            self.camera.position_.init_vector(self.mx, self.my, 0)
        elif view == 2:  # Top view Y
            print("Setting top view")
            direction = sl.Translation()
            direction.init_vector(0, -1, 0)  # Look towards the negative Y-axis
            vertical = sl.Translation()
            vertical.init_vector(0, 0, -1)  
            # vertical.init_vector(0, 0, -5*abs(self.mz)) 
            self.camera.setDirection(direction, vertical)
            print(str(self.mx) + " " + str(self.my) + " " + str(self.mz))
            self.camera.position_.init_vector(self.mx, 2*abs(self.mz), self.mz)
        elif view == 3:  # Side view X
            print("Setting side view")
            direction = sl.Translation()
            direction.init_vector(-1, 0, 0)  # Look towards the positive X-axis
            vertical = sl.Translation()
            vertical.init_vector(0, 0, 1)
            # vertical.init_vector(0, 0, 5*abs(self.mz))    
            self.camera.setDirection(direction, vertical)
            self.camera.position_.init_vector(2*abs(self.mz), self.my, self.mz)
